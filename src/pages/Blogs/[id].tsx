...
const BlogDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      const { data, error } = await supabase
        .from("blogs")
        .select(
          "id, title, description, image_url, category, tags, content, created_at"
        )
        .eq("id", id)
        .single();
  ...
  return (
    <>
      <Helmet>
        <title>{blog.title} | Daichiure Blog</title>
        <meta name="description" content={blog.description} />
        <meta
          name="keywords"
          content={`${blog.tags}, gaming blog, Daichiure, online games, gaming tips`}
        />
        <meta property="og:title" content={`${blog.title} | Daichiure Blog`} />
        <meta property="og:description" content={blog.description} />
        <meta
          property="og:image"
          content={blog.image_url || "https://www.daichiure.live/logo.png"}
        />
        <meta
          property="og:url"
          content={`https://www.daichiure.live/blogs/${id}`}
        />
        <meta property="og:type" content="article" />
        <meta property="article:published_time" content={blog.created_at} />
        <meta property="article:modified_time" content={blog.created_at} />
        <meta property="article:author" content="Daichiure" />
        <meta property="article:section" content={blog.category} />
        <meta property="article:tag" content={blog.tags} />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`${blog.title} | Daichiure Blog`} />
        <meta name="twitter:description" content={blog.description} />
        <meta
          name="twitter:image"
          content={blog.image_url || "https://www.daichiure.live/logo.png"}
        />
        <link rel="canonical" href={`https://www.daichiure.live/blogs/${id}`} />
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            url: `https://www.daichiure.live/blogs/${id}`,
            ...
            mainEntityOfPage: {
              "@type": "WebPage",
              "@id": `https://www.daichiure.live/blogs/${id}`,
            },
            keywords: blog.tags,
      ...
      <div
        className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-950 to-black py-10 px-4 sm:px-6 lg:px-8"
        style={fontStyle}
      >
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-10">
          ...
          <div className="w-full lg:w-3/4">
            <article className="w-full bg-gray-800 rounded-3xl shadow-xl p-8 sm:p-10 lg:p-12 border border-gray-700">
              <Link
                to="/blogs"
                className="text-amber-400 hover:text-amber-300 transition-colors duration-200 mb-6 inline-flex items-center group"
              >
                <svg
              ...
              <div className="flex items-center gap-3 mb-6 flex-wrap">
                {blog.category && (
                  <span className="bg-amber-600 text-white px-4 py-1.5 rounded-full text-sm font-semibold tracking-wide">
                    {blog.category}
                  </span>
                )}
                {blog.tags &&
                  blog.tags.split(",").map((tag) => (
                    <span
                      key={tag.trim()}
                      className="bg-gray-700 text-amber-300 px-3 py-1 rounded-full text-xs opacity-90"
                    >
                      {tag.trim()}
                    </span>
                  ))}
              ...
              {blog.image_url && (
                <img
                  src={blog.image_url}
                  alt={blog.title}
                  className="w-full rounded-xl mb-8 max-h-[400px] object-cover shadow-lg border border-gray-700"
                />
              )}
              ...
              <div className="mt-12 pt-8 border-t border-gray-700">
                ...
                <div className="mb-8 space-y-6">
                  ...
                  {comments.map((comment) => (
                    <div
                      key={comment.id}
                      className="p-5 bg-gray-900 rounded-xl shadow-md border border-gray-700"
                    >
          ...
          <aside className="w-full lg:w-1/4 space-y-8">
            <div className="bg-gray-900 rounded-2xl shadow-lg p-6 border border-gray-700">
              <h4 className="text-amber-400 text-xl font-bold mb-3">
                Sponsor Spotlight
              </h4>
              <p className="text-gray-400 text-base">
                Your brand could be featured here, reaching a highly engaged
                audience!
              </p>
