...
interface Fighter {
  ...
  isPlayer: boolean;
  facingRight: boolean;
  state: 'idle' | 'walking' | 'jumping' | 'punching' | 'kicking' | 'blocking' | 'special' | 'hit' | 'ko';
  stateTimer: number;
  combo: number;
  lastHit: number;
}
...
const Fighter: React.FC = () => {
  ...
  const [gameState, setGameState] = useState<'start' | 'fighting' | 'roundEnd' | 'gameOver'>('start');
  ...
  const updateGame = useCallback((dt: number) => {
    ...
    setPlayer(p => {
      ...
      // Screen bounds
      newPlayer.pos.x = Math.max(50, Math.min(GAME_WIDTH - 50, newPlayer.pos.x));
    ...
    // Update hitboxes
    setHitboxes(hitboxes => hitboxes.map(hitbox => {
      ...
      if (checkCollision(hitboxRect, {
        ...
        // Hit!
        if (target.isPlayer) {
          setPlayer(p => ({ ...p, health: Math.max(0, p.health - hitbox.damage), state: 'hit', stateTimer: 10 }));
        } else {
          setAi(a => ({ ...a, health: Math.max(0, a.health - hitbox.damage), state: 'hit', stateTimer: 10 }));
        }
  ...
  const createHitbox = (fighter: Fighter, attackType: string) => {
    const hitbox: Hitbox = {
      ...
      width: 40,
      height: 20,
      damage: attackType === 'punch' ? PUNCH_DAMAGE : 
              attackType === 'kick' ? KICK_DAMAGE : 
              attackType === 'uppercut' ? UPPERCUT_DAMAGE : SPECIAL_DAMAGE,
      active: true,
    };
  ...
  const draw = useCallback(() => {
    ...
    // Draw health bars
    const drawHealthBar = (x: number, y: number, health: number, isPlayer: boolean) => {
      const width = 200;
      const height = 20;
      ...
      ctx.fillText(`${isPlayer ? 'Player' : 'AI'}: ${health}`, x + width / 2, y + 15);
    };
    ...
    if (specialCooldown > 0) {
      ...
      ctx.textAlign = 'center';
      ctx.fillText(`Special Cooldown: ${Math.ceil(specialCooldown / 60)}s`, GAME_WIDTH / 2, 80);
    }
    ...
    ctx.fillText('Punch: J | Kick: K | Block: L | Special: U', 10, GAME_HEIGHT - 30);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-400 to-green-300 font-inter p-4">
      <h1 className="text-4xl font-bold text-white mb-6 drop-shadow-lg">Stickman Fighter</h1>
      <div className="relative bg-white rounded-xl shadow-2xl overflow-hidden border-4 border-gray-700 flex flex-col items-center p-2 md:p-4 lg:p-6" style={{ width: GAME_WIDTH, aspectRatio: `${GAME_WIDTH}/${GAME_HEIGHT}` }}>
        ...
        {gameState === 'start' && (
          <div className="absolute inset-0 bg-black bg-opacity-80 flex flex-col items-center justify-center rounded-lg">
            <h2 className="text-white text-5xl font-extrabold mb-6 animate-pulse">Stickman Fighter</h2>
            <p className="text-white text-xl mb-8 text-center max-w-md">
              Fight against the AI in this epic stickman battle!<br/>
