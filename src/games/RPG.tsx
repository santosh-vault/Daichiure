...
export const RPGGame: React.FC = () => {
  ...
  useGameCoins({
    ...
    score: player.level * 20 + player.gold, // Level bonus + gold collected
  ...
  const movePlayer = useCallback(
    (dx: number, dy: number) => {
      ...
      if (itemAtPosition) {
        setItems((prev) =>
          prev.filter((item) => !(item.x === newX && item.y === newY))
        );
  ...
  const handleBattle = useCallback(() => {
    ...
    if (newEnemyHealth <= 0) {
      ...
      setMessage(
        `Defeated ${currentEnemy.name}! +${experienceGained} XP, +${goldGained} Gold`
      );
      ...
      setCurrentEnemy((prev) =>
        prev ? { ...prev, health: newEnemyHealth } : null
      );
  ...
  const renderWorld = () => {
    const world = Array(WORLD_SIZE)
      .fill(null)
      .map(() => Array(WORLD_SIZE).fill(""));
    ...
    return (
      ...
      >
        ...
        <div style={{ color: "#fff", fontSize: 16 }}>
          A simple RPG adventure game. Explore the world, collect items, and
          battle monsters. Use WASD to move.
        </div>
      </div>
    );
  }
        ...
        {gameState === "battle" && currentEnemy && (
          <div className="text-center mb-6 p-4 bg-gray-800 rounded-lg border border-gray-600">
            ...
            <div className="text-gray-300 mb-4">
              ...
              <div>
                Enemy ATK: {currentEnemy.attack} | DEF: {currentEnemy.defense}
              </div>
            </div>
            <button
