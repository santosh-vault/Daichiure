...
const Football: React.FC = () => {
  ...
  const [score, setScore] = useState<{ player: number; ai: number }>({
    player: 0,
    ai: 0,
  });
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    if (gameState.currentTurn !== "player" || gameState.phase === "goal")
      return;
  ...
  const handleMouseUp = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (dragDistance > 10) {
      ...
      targetPos.x = clamp(
        targetPos.x,
        FIELD_MARGIN + PLAYER_RADIUS,
        GAME_WIDTH - FIELD_MARGIN - PLAYER_RADIUS
      );
      targetPos.y = clamp(
        targetPos.y,
        FIELD_MARGIN + PLAYER_RADIUS,
        GAME_HEIGHT - FIELD_MARGIN - PLAYER_RADIUS
      );
  ...
  const aiTurn = () => {
    ...
    setPlayers((prev) =>
      prev.map((p) =>
        p.id === selectedAI.id ? { ...p, targetPos: targetPos } : p
      )
    );
    ...
    const checkMovementComplete = () => {
      ...
      if (currentPlayer && currentPlayer.targetPos) {
        const distToTarget = distance(
          currentPlayer.pos,
          currentPlayer.targetPos
        );
        if (distToTarget > 8) {
          // Increased tolerance for reaching target
          ...
          const angle = Math.atan2(
            shotTarget.y - currentPlayer.pos.y,
            shotTarget.x - currentPlayer.pos.x
          );
        ...
        const shotTarget = calculateAIShotTarget(selectedAI);
        const angle = Math.atan2(
          shotTarget.y - selectedAI.pos.y,
          shotTarget.x - selectedAI.pos.x
        );
  ...
  const endTurn = () => {
    ...
    console.log(
      `Ending turn. Current: ${gameState.currentTurn}, New: ${newTurn}`
    ); // Debug log
  ...
  const selectBestAIPlayer = (aiPlayers: Player[]): Player => {
    ...
    const aiGoal = {
      x: GAME_WIDTH - FIELD_MARGIN - GOAL_WIDTH / 2,
      y: GAME_HEIGHT / 2,
    };
    ...
    const playerScores = aiPlayers.map((player) => {
      ...
      if (player.role === "forward" && player.pos.x > GAME_WIDTH / 2)
        score += 50;
      if (player.role === "goalkeeper" && player.pos.x > GAME_WIDTH * 0.7)
        score += 30;
    ...
    console.log(
      "AI player selection scores:",
      playerScores.map((p) => `${p.player.role}: ${p.score}`)
    );
  ...
  const calculateAIShotTarget = (aiPlayer: Player): Vec2 => {
    ...
    const finalTarget = {
      ...
      y: clamp(
        ...
        GAME_HEIGHT / 2 - GOAL_HEIGHT / 2 + 10,
        GAME_HEIGHT / 2 + GOAL_HEIGHT / 2 - 10
      ),
    };

    console.log(
      `AI ${aiPlayer.role} shooting at:`,
      finalTarget,
      `accuracy: ${accuracy}`
    );
    return finalTarget;
  ...
  const updateGame = (dt: number) => {
    ...
    setPlayers((prevPlayers) =>
      prevPlayers.map((player) => {
        ...
        // Clamp to field boundaries
        updatedPlayer.pos.x = clamp(
          updatedPlayer.pos.x,
          FIELD_MARGIN + PLAYER_RADIUS,
          GAME_WIDTH - FIELD_MARGIN - PLAYER_RADIUS
        );
        updatedPlayer.pos.y = clamp(
          updatedPlayer.pos.y,
          FIELD_MARGIN + PLAYER_RADIUS,
          GAME_HEIGHT - FIELD_MARGIN - PLAYER_RADIUS
        );
        ...
        if (
          updatedPlayer.team === "ai" &&
          updatedPlayer.targetPos &&
          !updatedPlayer.hasMoved
        ) {
    ...
    players.forEach((player) => {
      ...
      if (d < PLAYER_RADIUS + BALL_RADIUS) {
        ...
        // Push ball out
        nextBall.pos.x =
          player.pos.x + normal.x * (PLAYER_RADIUS + BALL_RADIUS + 1);
        nextBall.pos.y =
          player.pos.y + normal.y * (PLAYER_RADIUS + BALL_RADIUS + 1);
        ...
        const velocityAlongNormal =
          relativeVel.x * normal.x + relativeVel.y * normal.y;
    ...
    if (Math.abs(nextBall.vel.x) < 0.5 && Math.abs(nextBall.vel.y) < 0.5) {
      ...
      if (gameState.ballMoving && gameState.currentTurn === "player") {
        console.log(
          `Ball stopped moving. Current turn: ${gameState.currentTurn}`
        ); // Debug log
        setTimeout(() => {
          endTurn();
  ...
  const draw = useCallback(() => {
    ...
    ctx.strokeRect(
      FIELD_MARGIN,
      FIELD_MARGIN,
      GAME_WIDTH - 2 * FIELD_MARGIN,
      GAME_HEIGHT - 2 * FIELD_MARGIN
    );
    ...
    players.forEach((player) => {
      ...
      if (speed > 0.5) {
        ...
        ctx.lineTo(
          player.pos.x - player.vel.x * 3,
          player.pos.y - player.vel.y * 3
        );
    ...
    if (gameState.phase === "select" && gameState.currentTurn === "player") {
      ...
      ctx.fillStyle = "#fbbf24";
      ctx.fillText(
        "Click and drag a player to move and shoot",
        GAME_WIDTH / 2,
        160
      );
    } else if (
      ...
      ctx.fillStyle = "#fbbf24";
      ctx.fillText(
        "Drag opposite to desired trajectory, then release to shoot",
        GAME_WIDTH / 2,
        160
      );
    }
    ...
    if (ballSpeed > 0.1) {
      ...
      ctx.textAlign = "left";
      ctx.fillText(
        `Ball Speed: ${Math.round(ballSpeed)}`,
        10,
        GAME_HEIGHT - 30
      );
      ctx.fillText(
        `Ball Height: ${Math.round(GAME_HEIGHT - FIELD_MARGIN - ball.pos.y)}`,
        10,
        GAME_HEIGHT - 15
      );
      ctx.textAlign = "center";
  ...
  useGameCoins({
    gameId: "football",
    trigger: timer <= 0, // Game ends when timer reaches 0
    score: score.player * 100 + (score.player > score.ai ? 500 : 0), // Score based on goals + win bonus
    duration:
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-400 via-blue-500 to-purple-600 flex items-center justify-center p-4">
        <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 text-center max-w-md w-full">
          ...
          <p className="text-white/90 mb-8 text-lg">
            Turn-based football with 2 players per team! Click, drag, and shoot
            to score!
          </p>
          <button
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        <div
          className="relative bg-white rounded-xl shadow-2xl overflow-hidden border-4 border-green-700 flex flex-col items-center p-2 md:p-4 lg:p-6"
          style={{
            width: GAME_WIDTH,
        ...
        >
                ...
                {gameModes.map((mode) => (
                  ...
                  >
                    ...
                    <div className="text-sm opacity-90">
                      Duration: {mode.duration}s | AI Difficulty:{" "}
                      {mode.aiDifficulty}x
                    </div>
                  </button>
                ))}
          ...
          {gameState.phase === "goal" && (
            <div className="absolute inset-0 bg-black bg-opacity-60 flex flex-col items-center justify-center rounded-lg">
              <h2 className="text-yellow-400 text-6xl font-extrabold mb-4 animate-bounce">
                GOAL!
              </h2>
            </div>
          )}
          ...
          {gameState.phase === "end" && (
            <div className="absolute inset-0 bg-black bg-opacity-80 flex flex-col items-center justify-center rounded-lg">
              <h2 className="text-red-500 text-6xl font-extrabold mb-4">
                Full Time!
              </h2>
              ...
              <div className="text-white text-lg mb-6">
                <p>
                  Player Stats: {playerStats.goals} goals, {playerStats.shots}{" "}
                  shots
                </p>
                <p>
                  AI Stats: {aiStats.goals} goals, {aiStats.shots} shots
                </p>
              </div>
        ...
        <div className="text-center text-white/90 mt-8">
          ...
          <p className="text-sm">
            1. Click and drag a player to move and shoot in one action
          </p>
          <p className="text-sm">
            2. Drag opposite to desired trajectory, then release to shoot
          </p>
          <p className="text-sm">3. Take turns with the AI to score goals</p>
          <p className="text-sm">
            4. Each team has 2 players: G (Goalkeeper), F (Forward)
          </p>
          <p className="text-sm">5. Ball bounces off boundaries and players</p>
          <p className="text-sm mt-2">
            Fullscreen: <span className="text-yellow-300">F</span>
          </p>
        </div>
      </div>
    </div>
