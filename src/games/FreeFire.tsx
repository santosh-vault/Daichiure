...
export const FreeFireGame: React.FC = () => {
  ...
  const updateCamera = () => {
    ...
    camera.current.x = Math.max(
      0,
      Math.min(MAP_WIDTH - CANVAS_WIDTH, camera.current.x)
    );
    camera.current.y = Math.max(
      0,
      Math.min(MAP_HEIGHT - CANVAS_HEIGHT, camera.current.y)
    );
  };
  ...
  const updateUI = () => {
    if (healthRef.current)
      healthRef.current.textContent = String(player.current.health);
    if (ammoRef.current)
      ammoRef.current.textContent = String(player.current.ammo);
    if (scoreRef.current)
      scoreRef.current.textContent = String(player.current.score);
    if (levelRef.current)
      levelRef.current.textContent = String(difficultyLevel);
    if (reloadRef.current)
      reloadRef.current.style.display = player.current.reloading
        ? "block"
        : "none";
  ...
  const drawPlayer = (ctx: CanvasRenderingContext2D) => {
    ...
    if (
      ...
      Math.floor(player.current.invulnerable / 3) % 2 === 0
    ) {
    ...
    if (player.current.powerUpActive > 0) {
      ctx.save();
      ctx.shadowColor =
        player.current.powerUpType === "rapid" ? "#ff0" : "#0ff";
      ctx.shadowBlur = 20;
    }
    ...
    ctx.beginPath();
    ctx.arc(
      screenPos.x + player.current.w / 2,
      screenPos.y + 15,
      12,
      0,
      Math.PI * 2
  ...
  const drawMapDecorations = (ctx: CanvasRenderingContext2D) => {
    ...
    buildings.forEach((building) => {
      ...
      if (
        screenPos.x + building.w > 0 &&
        screenPos.x < CANVAS_WIDTH &&
        screenPos.y + building.h > 0 &&
    ...
    trees.forEach((tree) => {
      ...
      if (
        screenPos.x > -50 &&
        screenPos.x < CANVAS_WIDTH + 50 &&
        screenPos.y > -50 &&
        screenPos.y < CANVAS_HEIGHT + 50
      ) {
        ctx.save();
        // Tree trunk
  ...
  const update = () => {
    ...
    player.current.x = Math.max(
      0,
      Math.min(MAP_WIDTH - player.current.w, player.current.x)
    );
    player.current.y = Math.max(
      0,
      Math.min(MAP_HEIGHT - player.current.h, player.current.y)
    );
    ...
    bullets.current = bullets.current.filter(
      (b) =>
        b.x > -20 && b.x < MAP_WIDTH + 20 && b.y > -20 && b.y < MAP_HEIGHT + 20
    );
    ...
    bosses.current.forEach((boss) => {
      ...
      if (boss.phase === 0) {
        ...
        boss.x =
          player.current.x + player.current.w / 2 + Math.cos(angle) * 150;
    ...
    ammos.current.forEach((a, ai) => {
      if (
        ...
        player.current.ammo = Math.min(
          player.current.ammo + AMMO_PICKUP_AMOUNT,
          MAX_AMMO
        );
        updateUI();
      }
    ...
    // Boss spawn check
    if (
      player.current.score >= lastBossSpawn.current + BOSS_SPAWN_SCORE &&
      !bossActive
    ) {
      spawnBoss();
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          Position: ({Math.floor(player.current.x)},{" "}
          {Math.floor(player.current.y)})
        </div>
        <div
          ref={reloadRef}
          style={{
        ...
        {player.current.powerUpActive > 0 && (
          <div
            style={{
              color: player.current.powerUpType === "rapid" ? "#ff0" : "#0ff",
              fontWeight: "bold",
              marginTop: 4,
            }}
          >
