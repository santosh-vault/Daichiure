...
export const AllOfUsAreDead: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      if (
        !level2SpawnedRef.current &&
        gameTimeRef.current >= SPAWN_LEVEL2_AFTER * 30
      ) {
        spawnLevel2Mobs();
      ...
      player.x = Math.max(
        MOB_SIZE,
        Math.min(CANVAS_WIDTH - MOB_SIZE, player.x)
      );
      player.y = Math.max(
        MOB_SIZE,
        Math.min(CANVAS_HEIGHT - MOB_SIZE, player.y)
      );
      ...
      nonPlayerAllies.forEach((ally, index) => {
        ...
        if (dist > 5) {
          ...
          ally.x = Math.max(
            MOB_SIZE,
            Math.min(CANVAS_WIDTH - MOB_SIZE, ally.x)
          );
          ally.y = Math.max(
            MOB_SIZE,
            Math.min(CANVAS_HEIGHT - MOB_SIZE, ally.y)
          );
        }
      });
      ...
      groupLeaders.forEach((leader) => {
        leader.x += leader.vx;
        leader.y += leader.vy;
        if (leader.x <= MOB_SIZE || leader.x >= CANVAS_WIDTH - MOB_SIZE)
          leader.vx *= -1;
        if (leader.y <= MOB_SIZE || leader.y >= CANVAS_HEIGHT - MOB_SIZE)
          leader.vy *= -1;
      });
      ...
      for (const neutral of neutrals) {
        ...
        if (dist < MOB_SIZE * 1.5 && neutral.groupId) {
          const groupMobs = neutrals.filter(
            (m) => m.groupId === neutral.groupId
          );
  ...
  const draw = () => {
    ...
    for (const mob of mobsRef.current) {
      ...
      if (mob.fighting && mob.fightFrame !== undefined && mob.fightTarget) {
        ...
        const angle = Math.atan2(
          mob.fightTarget.y - mob.y,
          mob.fightTarget.x - mob.x
        );
      ...
      if (!mob.isAlly && mob.groupId) {
        const groupSize = mobsRef.current.filter(
          (m) => m.groupId === mob.groupId
        ).length;
        const groupLeader = mobsRef.current.find(
          (m) =>
            m.groupId === mob.groupId &&
            !mobsRef.current.some(
              (other) =>
    ...
    if (player) {
      ...
      ctx.textAlign = "center";
      ctx.fillText(
        `Team: ${mobsRef.current.filter((m) => m.isAlly).length}`,
        player.x,
        player.y - MOB_SIZE - 15
      );
    }
  };
      ...
      <div className="flex flex-col items-center justify-center h-full bg-gray-900 text-white p-8">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-300 mb-6">
            Recruit allies and survive! Move with arrow keys or WASD. Fight
            level 1 mobs (2s) and level 2 mobs (4s) to convert them. Your allies
            will fight automatically. Touch bigger groups and you'll DIE! Level
            2 mobs spawn after 30s. Reach {MAX_TEAM} allies to win!
          </p>
          <button
            onClick={handleStart}
          ...
          <div className="mt-4 text-center text-gray-400 text-sm max-w-md">
            ...
            <p>
              <strong>Goal:</strong> Fight level 1 mobs (gray, 2s) and level 2
              mobs (red, 4s) to recruit them!
            </p>
            <p>
              <strong>Strategy:</strong> Multiple allies can fight
              simultaneously. Avoid larger groups - instant death!
            </p>
            <p>
              <strong>Win Condition:</strong> Reach {MAX_TEAM} team members
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="text-center max-w-md">
          {gameStats.won ? (
            <>
              ...
              <p className="text-gray-300 mb-6">
                Congratulations! You've successfully recruited{" "}
                {gameStats.teamSize} allies and survived the apocalypse!
              </p>
            </>
          ) : (
            <>
              ...
              <p className="text-gray-300 mb-6">
                You were defeated! Your team size was {gameStats.teamSize}. Try
                again to reach {MAX_TEAM} allies!
              </p>
            </>
          )}
          <button
          ...
          <div className="mt-4 text-center text-gray-400 text-sm">
            <p>
              <strong>Final Team Size:</strong> {gameStats.teamSize}
            </p>
            <p>
              <strong>Target:</strong> {MAX_TEAM} allies
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full bg-gray-900 text-white">
      ...
      <div className="mt-4 text-center text-gray-400 text-sm max-w-md">
        ...
        <p>
          <strong>Goal:</strong> Fight level 1 mobs (gray, 2s) and level 2 mobs
          (red, 4s) to recruit them!
        </p>
        <p>
          <strong>Strategy:</strong> Multiple allies can fight simultaneously.
          Avoid larger groups - instant death!
        </p>
        <p>
          <strong>Win Condition:</strong> Reach {MAX_TEAM} team members
        </p>
