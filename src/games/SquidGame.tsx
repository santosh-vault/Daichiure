...
const getRandomShape = () =>
  dalgonaShapes[Math.floor(Math.random() * dalgonaShapes.length)];
...
const SquidGame: React.FC = () => {
  ...
  // Game start time for coin system
  const [gameStartTime] = useState(Date.now());

  // Coin earning system - triggers when game is won (all levels completed)
  useGameCoins({
    gameId: "SquidGame",
    trigger: status === "won" && level === 2, // Complete all 3 levels
    score: (level + 1) * 50, // 50 coins per level completed
  ...
  const handleDalgona = (e: React.MouseEvent) => {
    ...
    if (clickPercent >= precision && clickPercent <= precision + 5) {
      // 5% zone
      setProgress((p) => {
        if (p >= 100) {
          setShowTransition(true);
  ...
  if (status === "lost") {
    return (
      <div
        className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-950 to-black flex flex-col items-center justify-center"
        style={fontStyle}
      >
        <div className="flex flex-col items-center">
          <span className="text-8xl mb-6">❌</span>
          {timedOut && level === 1 && (
            <div className="mb-2 text-2xl text-red-400 font-bold animate-pulse">
              Time’s Up!
            </div>
          )}
          <div className="mb-4 text-xl text-white text-center max-w-lg">
            {instructions[level]}
          </div>
          <button
            onClick={handleRestart}
            className="bg-amber-500 text-gray-900 px-8 py-4 rounded-2xl font-bold text-2xl hover:bg-amber-600 transition-colors"
          >
            Restart
          </button>
        </div>
  ...
  return (
    <div
      className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-950 to-black flex flex-col items-center justify-center py-8 px-2"
      style={fontStyle}
    >
      <div className="mb-6 text-xl text-white text-center max-w-lg">
        {instructions[level]}
      </div>
      ...
      {level === 0 && (
        <div className="w-full max-w-xl flex flex-col items-center">
          <div className="flex items-center justify-between w-full mb-6">
            ...
            <span
              className={`text-6xl px-6 py-2 rounded-2xl font-bold ${
                light === "green"
                  ? "bg-green-500 text-gray-900"
                  : light === "fakegreen"
                  ? "bg-green-300 text-gray-900 border-4 border-red-500"
                  : "bg-red-500 text-white"
              }`}
            >
              {light === "green" ? "🟢" : light === "fakegreen" ? "🟢" : "🔴"}
            </span>
            <span className="text-7xl">👧</span>
          </div>
          <div className="relative w-full h-12 bg-gray-800 rounded-full mb-4">
            <div
              className="absolute left-0 top-0 h-12 bg-amber-400 rounded-full transition-all duration-300"
              style={{ width: `${playerPos}%`, maxWidth: "100%" }}
            ></div>
            <span
              className="absolute left-0 top-0 h-12 flex items-center justify-center"
              style={{ left: `calc(${playerPos}% - 24px)` }}
            >
          ...
          <button
            onClick={handleRedLightGreenLight}
            className="w-full bg-amber-500 text-gray-900 px-8 py-4 rounded-2xl font-bold text-2xl hover:bg-amber-600 transition-colors mt-2"
          >
            ▶️
          </button>
        </div>
      ...
      {level === 1 && (
        <div className="w-full max-w-xl flex flex-col items-center">
          ...
          <div
            className={`relative w-full h-10 bg-gray-800 rounded-full mb-2 cursor-pointer transition-transform duration-150 ${
              crackAnim ? "animate-shake" : ""
            }`}
            onClick={handleDalgona}
            style={{ position: "relative" }}
          >
            <div
              className="absolute top-0 left-0 h-10 bg-amber-400 rounded-full transition-all duration-300"
              style={{ width: `${progress}%` }}
            ></div>
            {/* Precision zone */}
            <div
              className="absolute top-0 h-10 rounded-full border-4 border-green-400 opacity-80"
              style={{
                left: `${precision}%`,
                width: "5%",
                pointerEvents: "none",
            ...
            {crackMeter > 60 && (
              <>
                <div
                  className="absolute inset-0 flex items-center justify-center pointer-events-none select-none text-4xl animate-pulse"
                  style={{ zIndex: 2 }}
                >
                  💥
                </div>
                ...
                >
                  <polyline
                    points="0,10 10,20 20,5 30,25 40,10 50,20 60,5 70,25 80,10 90,20 100,5 110,25 120,10 130,20 140,5 150,25 160,10 170,20 180,5 190,25 200,10"
                    fill="none"
                    stroke="#fff"
                    strokeWidth="3"
                    opacity="0.7"
          ...
          {crackAnim && (
            <div className="text-red-400 font-bold text-lg animate-pulse mb-1">
              Cracked!
            </div>
          )}
        ...
        <div className="w-full max-w-xl flex flex-col items-center">
          <div className="flex items-center justify-between w-full mb-4">
            ...
            <div className="flex-1 mx-2 relative h-10 bg-gray-800 rounded-full">
              <div
                className="absolute top-0 h-10 bg-amber-400 rounded-full transition-all duration-300"
                style={{ left: 0, width: `${ropePos}%` }}
              ></div>
              <span
                className="absolute left-0 top-0 h-10 flex items-center justify-center"
                style={{ left: `calc(${ropePos}% - 24px)` }}
              >
                <span className="text-4xl">🤼‍♂️</span>
              </span>
            </div>
